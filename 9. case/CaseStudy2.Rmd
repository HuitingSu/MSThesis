...
```{r}
lasso_sign <- logical(100)
for(i in 1:100){
    getXy()
    avgy= y %*% c(0.15, 0.35, 0.3, 0.2)
    cv.fit = cv.glmnet(X, avgy, intercept = FALSE)
    b_hat = coef(cv.fit)[-1]
    if(length(which(b_hat != 0))<2)lasso_sign[i] =FALSE
    else lasso_sign[i] = which(b_hat != 0) == which(sign(b[,1])!=0)
}
```
